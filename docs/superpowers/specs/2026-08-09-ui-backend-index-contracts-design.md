...
记录；磁盘原文件保持不变。删除成功后刷新检索服务，使关键词索引立即同步。
...
2. 索引服务测试证明默认模式仍跳过未变化文件，而 `force=true` 会重新嵌入和写入。
3. 任务仓库测试覆盖逐文件失败状态与受控/意外任务级错误。
4. API 测试覆盖四个端点、参数校验、删除后刷新、重建任务参数、原文件缺失、
   结构化 404/503 和失败序列化。
5. 运行聚焦测试、完整后端测试和 `git diff --check`。
...
6. 完整测试套件通过，且提交不包含用户文件、模型、数据库或缓存。
