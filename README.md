...
Sentence Transformers、MobileCLIP 和 Chroma；前端使用 Flutter，提供搜索、索引库管理、
设置持久化和无障碍支持。运行时不需要把用户文件或查询发送到远程服务。
...
| 图片解析 | `.jpg`、`.jpeg`、`.png` |
| 检索通道 | 关键词 BM25、文本语义、图文语义，可按权重使用加权 RRF 融合 |
...
- `models/model-manifest.json` 及其声明的两个本地模型均已就绪并通过 SHA-256 校验。
