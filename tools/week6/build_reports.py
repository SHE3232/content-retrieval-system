...
def _build_performance_report(output: Path, context: dict[str, Any], comparison: dict[str, Any]) -> None:
    ...
    doc.add_paragraph("基线与候选必须使用同一硬件、电源模式、数据哈希和模型哈希；每轮预热不少于 10 次，正式测量不少于 100 次，共 3 轮，以中位数比较。")
    doc.add_heading("比较结果", level=1)
    improvements = comparison.get("improvements_percent", {})
    rows = [(name, f"{value:.3f}%" if isinstance(value, (int, float)) else "证据待生成") for name, value in improvements.items()]
    if not rows:
        rows = [("性能比较", "证据待生成")]
    _add_table(doc, ("指标", "改善"), rows, [4800, 4560])
    doc.add_heading("准确率防回归", level=1)
    doc.add_paragraph("NQ 与 COCO 的 recall@10、MRR@10、NDCG@10 相对基线下降均不得超过 0.01。")
    doc.add_heading("结论", level=1)
    doc.add_paragraph(f"G6 当前状态：{context['gate_statuses'].get('G6', 'MISSING')}。没有可比基线或任一指标不足 5% 时，本报告不得宣称性能验收通过。")
    output.parent.mkdir(parents=True, exist_ok=True)
    doc.save(output)
...
def _build_security_report(output: Path, context: dict[str, Any], bugs: dict[str, Any], security: dict[str, Any]) -> None:
    ...
    rows = [(item.get("id"), item.get("status")) for item in checks if isinstance(item, dict)] or [("安全审查", "证据待生成")]
    _add_table(doc, ("检查", "状态"), rows, [6000, 3360])
