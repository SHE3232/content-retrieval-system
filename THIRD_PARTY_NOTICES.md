...
上述表格不是传递依赖的删节许可清单。4 套 Python `uv.lock` 和 1 套 Flutter `pubspec.lock` 中的全部 312 条锁定依赖记录已经逐项核查；以 [dependency-licenses.csv](docs/dependency-licenses.csv) 为完整清单。发行归档必须同时保留各 Python wheel、Dart 包和随附原生库中提供的 `LICENSE`、`COPYING`、`NOTICE` 及版权文件，不能只保留本文件。
...
4 套 Python `uv.lock` + 1 套 Flutter `pubspec.lock`，共 312 条锁定依赖记录（Python 273，Dart 39）。
